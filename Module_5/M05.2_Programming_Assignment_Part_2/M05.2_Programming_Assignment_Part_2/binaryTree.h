...
public:
    ...
    //Function to determine the number of nodes with a single child.
    //Postcondition: Returns the number of single-parent nodes.
...
private:
    ...
    //Helper function to count single-parent nodes.
    //Generated by ChatGPT. 
};
...
/* These methods return the number of nodes that have a single child. Code generated by ChatGPT. */
template <class elemType>
int binaryTreeType<elemType>::singleParent(nodeType<elemType>* p) const
{
    if (p == nullptr)
        return 0; // Base case: no node, no single parent
...
template <class elemType>
int binaryTreeType<elemType>::treeSingleParentCount() const
{
    return singleParent(root); // Call helper function starting at root
}
